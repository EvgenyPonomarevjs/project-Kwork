...
$(document).ready(function () {
    ...
    function header_scroll() {
        ...
        if (scroll_top >= ($('.third').offset().top - $('.third').height()) && $(window).width() <= 1024) {
    ...
    function thirdTableSlider() {
        if ($('.third__table-carousel').length > 0) {
            var $slider = $('.third__table-carousel');
    ...
    if ($('.fifth__slider').length > 0) {
        var $slider = $('.fifth__slider');
        $slider.owlCarousel({
            loop: false,
            items: 1,
            responsive: {
    ...
    if ($('.eighth__selector-slider').length > 0) {
        var $slider = $('.eighth__selector-slider');
        $slider.owlCarousel({
            loop: false,
            nav: true,
            dots: false,
    ...
    if ($('.tenth__slider').length > 0) {
        var $slider = $('.tenth__slider');
        $slider.owlCarousel({
            loop: true,
            responsive: {
                0: {
    ...
    if ($('.fifteenth__list').length > 0) {
        var $slider = $('.fifteenth__list');
        $slider.owlCarousel({
            loop: false,
            responsive: {
                0: {
    ...
    $('.eighth__calc-natural-item').on('click', function () {
    ...
    $(document).on('click', function (e) {
        ...
        if ($elem.hasClass('eighth__selector-slider-item-img')) {
            var $item = $elem.closest('.eighth__selector-slider-item');

            var $preview = $('.eighth__calc-top-img-inner');
            var img_url = $elem.prop('src');
            $preview.css('background', 'url(' + img_url + ')');
            $preview.attr('href', img_url);

            $(document).find('.eighth__selector-slider-item').removeClass('active');
    ...
    function calcColor() {
        var color = $('.eighth__selector-slider-item.active').find('.eighth__selector-slider-item-img').data('color-name');
        var material = $('.eighth__selector-slider-item.active').find('.eighth__selector-slider-item-img').data('material-name');
        var $color_input = $('.eighth').find('input[name="color"]');
        var $material_input = $('.eighth').find('select[name="material"]');
        $color_input.val(color);
        $material_input.val(material);
        $material_input.selectmenu("refresh");
        calcValues();
    };
    ...
    function calcValues() {
        var $material_input = $('.eighth').find('select[name="material"]');
        var $thickness_input = $('.eighth').find('select[name="thickness"]');
        var material_thickness_price_val;
        var material_thickness_price_for_modal = '';
        var $work_input = $('.eighth').find('select[name="work"]');
        var work_price_val;
        var work_price_for_modal = '';
        var natural_price_val;
        var natural_price_for_modal = '';

        $material_input.on('change', function () {
            calcPrice();
        });

        $thickness_input.on('change', function () {
            calcPrice();
        });

        $work_input.on('change', function () {
            calcPrice();
        });
        ...
        function calcPrice() {
            ...
            var color = ', цвет: ' + $('input[name="color"]').val();

            if ($material_input.val() !== 'option-placeholder' && $thickness_input.val() !== 'option-placeholder') {
                material_thickness_price_val = material_price[$material_input.val()][$thickness_input.val()];
                material_thickness_price_for_modal = 'Материал: ' + $material_input.find(":selected").text() + ', толщина: ' + $thickness_input.find(":selected").text();
            } else {
                check = false;
            }

            if ($work_input.val() !== 'option-placeholder') {
                work_price_val = work_price[$work_input.val()];
                work_price_for_modal = ', установка: ' + $work_input.find(":selected").text();
            } else {
                check = false;
            }

            natural_price_val = 0;
            natural_price_for_modal = ', дополнительно: ';

            $('.eighth__calc-natural-item.active').each(function () {
                natural_price_val = natural_price_val + parseInt($(this).find('.eighth__calc-natural-item-value').text());
                natural_price_for_modal = natural_price_for_modal + ', ' + $(this).find('.eighth__calc-natural-item-name').text().replace(/\s+/g, " ");
            });

            if (check === true) {
                $('.eighth__calc-result').addClass('active');
                $('.eighth__calc-result-value-inner').text(natural_price_val + work_price_val + material_thickness_price_val);
                $('input[name="calc_price"]').val(natural_price_val + work_price_val + material_thickness_price_val);
                $('input[name="calc_desc"]').val(material_thickness_price_for_modal + work_price_for_modal + color + natural_price_for_modal);
            } else {
                $('.eighth__calc-result').removeClass('active');
            }

        }

        $('.eighth__calc-button').on('click', function () {
            modals.open('modal_lead');
            $('input[name="modal_calc_price"]').val($('input[name="calc_price"]').val());
            $('input[name="modal_calc_desc"]').val($('input[name="calc_desc"]').val());
            $('input[name="modal_form_name"]').val('Калькулятор');
        });
    ...
    $(".modal__form-button").on('click', function () {
        ...
        if ($(this).hasClass('disabled') !== true) {
            ...
            validate_forms.get('modal-form').validate_with_error();
        }
